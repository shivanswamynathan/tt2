...
class OrchestratorAgent:
    ...
    def handle_button_node(self, state: OrchestratorState) -> Dict[str, Any]:
        ...
        if user_query in ["more_examples", "more examples"]:
            ...
            if relevance == "RELEVANT":
                ...
                logger.info(f"Generated custom response for {title}: {response_message[:100]}...")
            if not isinstance(response_message, str) or not response_message.strip():
                logger.warning(f"Invalid response format for {title}")
                response_message = "I'm not sure how to respond to that. Could you clarify or choose an option?"
            next_action = "continue"
    ...
    def handle_qa_node(self, state: OrchestratorState) -> Dict[str, Any]:
        ...
        if relevance == "RELEVANT":
            ...
            logger.info(f"Generated custom response for {current_concept}: {combined[:100]}...")
        ...
        if not isinstance(combined, str) or not combined.strip():
            ...
            combined = "I'm not sure how to respond to that. Could you clarify or choose an option?"
    ...
    def handle_custom_node(self, state: OrchestratorState) -> Dict[str, Any]:
        ...
        logger.info(f"Generated custom response for {current_concept}: {response[:100]}...")
        
        if not isinstance(response, str) or not response.strip():
            logger.warning(f"Invalid custom response format for {current_concept}")
            response = "I'm not sure how to respond to that. Could you clarify or choose an option?"
