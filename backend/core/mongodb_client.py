...
class MongoDBClient:
    ...
    def get_available_topics(self) -> List[Dict[str, Any]]:
        ...
        """
        try:
            # Get distinct chapters and topics from updated schema
            pipeline = [
                {"$group": {
                    "_id": {
            ...
            for topic_data in topics_data:
                ...
                title = topic_data["_id"]["title"]
                subtopic_count = topic_data["subtopic_count"]
                chapter_index = topic_data["chapter_index"]
                chunk_index = topic_data["chunk_index"]
                description = topic_data["description"]
                
                # Create full topic identifier
                full_topic = f"{chapter} - {title}"
                
                topic_config = Config.get_topic_config(full_topic)
                
                topic_details.append({
                    "topic": full_topic,  
                    "display_name": title.replace("**", ""),  # Remove markdown formatting
    ...
    def get_topic_subtopics(self, topic_title: str) -> List[Dict[str, Any]]:
        ...
        """
        try:
            # Clean the topic title by removing markdown formatting
    ...
    def get_topic_content(self, topic: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        try:
            # Extract title from full topic string and clean it
            if " - " in topic:
                title = topic.split(" - ", 1)[1]
            else:
                title = topic
            ...
            subtopics = self.get_topic_subtopics(clean_title)
